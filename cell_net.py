...
class Encoder(chainer.Chain):
    def __init__(self, in_ch):
        layers = {}
        w = chainer.initializers.Normal(0.02)
        layers['c0'] = L.Convolution2D(in_ch, 32, 3, 1, 1, initialW=w)
       # layers['c1'] = CBR(16, 32, bn=True, sample='down', activation=F.leaky_relu, dropout=False)
        layers['c1'] = CBR(32, 64, bn=True, sample='down', activation=F.leaky_relu, dropout=False)
        layers['c2'] = CBR(64, 128, bn=True, sample='down', activation=F.leaky_relu, dropout=False)
        layers['c3'] = CBR(128, 256, bn=True, sample='down', activation=F.leaky_relu, dropout=False)
        layers['c4'] = CBR(256, 512, bn=True, sample='down', activation=F.leaky_relu, dropout=False)
...
class Decoder(chainer.Chain):
    def __init__(self, out_ch):
        ...
        layers['c3'] = CBR(1024, 512, bn=True, sample='up', activation=F.relu, dropout=True)
        layers['c4'] = CBR(768, 512, bn=True, sample='up', activation=F.relu, dropout=False)
        layers['c5'] = CBR(640, 256, bn=True, sample='up', activation=F.relu, dropout=False)
        layers['c6'] = CBR(320, 128, bn=True, sample='up', activation=F.relu, dropout=False)
        #layers['c7'] = CBR(160, 64, bn=True, sample='up', activation=F.relu, dropout=False)
        layers['c7'] = L.Convolution2D(80, out_ch, 3, 1, 1, initialW=w)
...
class Discriminator(chainer.Chain):
    def __init__(self, in_ch, out_ch):
        ...
        layers['c3'] = L.Convolution2D(512, 1, 3, 1, 1, initialW=w)
