...
class FacadeUpdater(chainer.training.StandardUpdater):
    ...
    def update_core(self):        
        ...
        eval_interval = 100
        if self.iteration % eval_interval == 0 and self.iteration > 0:
            eval_fp = open("result_20200121-MCF7set/eval_log.txt", "a")
            ...
            eval_fp.write('{}\t{}\t\t{}\t\t{}\n'.format(self.epoch, self.iteration, eval_enc(self.loss_enc, enc, x_out, t_out, y_fake),eval_dis(self.loss_dis, dis, y_real, y_fake)))
            eval_fp.close()
